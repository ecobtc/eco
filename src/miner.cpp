...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, CTxDestination address, CKey key, bool fMineWitnessTx)
{
    ...
    {
        LOCK(cs_main);
        SignCoinbaseTx(pblock, chainActive.Tip(), address, key, nExtraNonce);
    }
